...
#pragma warning restore 0649
	...
	private void GenerateEdges( VertexHelper vh )
	{
		...
		for (int j = 1; j >= -1; j -= 2)
		{
			...
			innerWidth = width - edgeThickness * j;
			innerHeight = height - edgeThickness * j;
			
			for (int i = 0; i < detail + 1; i++, triangleIndex += 2)
			{
				float radians = i * deltaRadians;
				...
				Vector3 outerPosition = new Vector3(cos * width + deltaWidth, sin * height + deltaHeight, 0f);
				// Calculate inner vertex position
				Vector3 innerPosition = new Vector3(cos * innerWidth + deltaWidth, sin * innerHeight + deltaHeight, 0f);

				// Alpha gradient for anti-aliasing effect (adjust 255 value for more fade)
				Color32 outerColor = new Color32(color32.r, color32.g, color32.b, 0);

				// Add vertices with fade on outer edge
				vh.AddVert(j == -1 ? outerPosition : innerPosition, j == 1 ? outerColor : color32, uv);
				vh.AddVert(j == -1 ? innerPosition : outerPosition, j == 1 ? color32 : outerColor, uv);
