...
public class TerrainMeshGenerator : MonoBehaviour
{
    ...
    public void GenerateMap()
    {
        ...
        for (int i = 0; i < 100; i++) {
            _gradientColorArray[i] = heightmapGradient.Evaluate(i / meshVariables.height);
        }
...
public struct HeightMapGenerator : IJobParallelFor {
    ...
    {
        float noiseVal = 0, amplitude = 1, freq = _heightmapVariables.ridgeNoiseScale, weight = 1;

        for (int o = 0; o < _heightmapVariables.octaves; o++)
        {
            float v = 1 - (1 - Mathf.Abs(noise.snoise(_meshVariables.seed * 0.01f + pos / freq / _meshVariables.terrainMeshDetail)));
            v *= v;
            v *= weight;
    ...
    float OctavedSimplexNoise(float2 pos)
    {
        ...
        for (int o = 0; o < _heightmapVariables.octaves; o++)
        {
            float v = 1 - (noise.snoise(_meshVariables.seed * 0.01f + pos / freq / _meshVariables.terrainMeshDetail) + 1) * 0.5f;
            noiseVal += v * amplitude;
